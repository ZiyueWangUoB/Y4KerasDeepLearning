...
#for filename in glob.glob('stats/*.npy'):
    ...
    if abs(stats[3]) > 2e-4 and abs(stats[4]) > 2e-4:
        p += 1
    elif abs(stats[3]) < 2e-4 and abs(stats[4]) < 2e-4:
        p -= 1
...
plt.figure()
plt.hist([points, pointsGood],bins=3)
plt.legend(['Bad','Good','combo'])
plt.title('pointsdxdy')


#Find hypot of dxdy 
hyp = np.hypot(dx,dy)
hypGood = np.hypot(dxGood,dyGood)
dxdy_mean = np.mean([dx,dy],axis=0)
dxdy_meanGood = np.mean([dxGood,dyGood],axis=0)
#plt.figure()
#plt.hist(sigma,bins=10)
#plt.title('sigma')

plt.figure()
plt.hist([bkg, bkgGood, bkg+bkgGood],bins=10)
plt.title('bkg')
...
plt.figure()
plt.hist([dxdy_mean, dxdy_meanGood],bins=10)
plt.legend(['Bad','Good','combo'])
plt.title('dxdymean')
